...
| Individual History Items | Processing history in memory (service) may not be as expensive                             | Need product approval to limit the number of history items (assets). For product, an item would mean an aggregation (example, at the show level) or for a sport. This is very difficult to manage within a single list. |
